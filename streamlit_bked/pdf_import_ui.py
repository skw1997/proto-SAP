            ...
            for uploaded_file in st.session_state.uploaded_files:
                with st.spinner(f"正在处理PDF文件: {uploaded_file.name}..."):
                    try:
                        ...
                        # 根据选择的公司处理PDF
                        if selected_company == "Wefaricate":
                            # 处理Wefaricate PDF
                            data = extract_wefaricate_data(tmp_file_path)
                            if data:
                                # 显示提取的数据
                                ...
                                # 插入到WF Open表
                                insert_wf_open_data(data)
                                st.success(f"✅ 成功导入 {len(data)} 条数据到 WF Open 表")
                                processed_files.append(uploaded_file.name)  # 标记为已处理
                            else:
                                st.warning(f"⚠️ 未从PDF {uploaded_file.name} 中提取到有效数据")
                                
                        elif selected_company == "Centurion Safety Products":
                            # 处理Centurion PDF
                            data = extract_centurion_data(tmp_file_path)
                            if data:
                                # 显示提取的数据
                                st.subheader(f"提取的数据预览 - {uploaded_file.name}")
                                df = pd.DataFrame(data)
                                ...
                                # 插入到Non-WF Open表
                                insert_non_wf_open_data(data)
                                st.success(f"✅ 成功导入 {len(data)} 条数据到 Non-WF Open 表")
                                processed_files.append(uploaded_file.name)  # 标记为已处理
                            else:
                                st.warning(f"⚠️ 未从PDF {uploaded_file.name} 中提取到有效数据")
